...
maak_leeg_plot <- function(bericht = "Geen data beschikbaar") {
  plot_ly() |>
    layout(
      paper_bgcolor = "rgba(0,0,0,0)",
      plot_bgcolor = "rgba(0,0,0,0)",
...
leeg_verenigingen <- data.frame(
  vereniging_id = character(0),
  vereniging = character(0),
  sport = character(0),
  aantal_leden = numeric(0),
  aantal_members = numeric(0),
  datum = as.Date(character(0)),
  pricing_code = character(0),
  vereniging_deal = character(0),
  discount = numeric(0),
  omzet = numeric(0)
)
...
maak_verenigingen_dataset <- function(verenigingen, verenigingen_pricing) {
  ...
  namen <- names(verenigingen)
  names(verenigingen) <- gsub("\\.", " ", namen)
  ...
  if (nrow(verenigingen_pricing) == 0) {
    return(verenigingen |>
             transmute(
               vereniging_id, vereniging, sport, aantal_leden, aantal_members,
               datum = as.Date(NA), pricing_code = NA_character_,
               vereniging_deal = NA_character_, discount = 0, omzet = 0
             ))
  }
  
  namen <- names(verenigingen_pricing)
  names(verenigingen_pricing) <- gsub("\\.", "_", namen)
  
  pricing <- verenigingen_pricing |>
    transmute(
      klantnummer = as.character(.data[["klantnummer"]]),
  ...
  match_index <- vapply(pricing$pricing_match, function(code) {
    hits <- which(
      verenigingen$vereniging_id_match == code |
  ...
  pricing_gematcht <- bind_cols(
    verenigingen[match_index, c("vereniging_id", "vereniging", "sport",
                                "aantal_leden", "aantal_members")],
    pricing |> select(datum, pricing_code, vereniging_deal, discount, omzet)
  )
  
  verenigingen_zonder_pricing <- verenigingen |>
  ...
  bind_rows(pricing_gematcht, verenigingen_zonder_pricing) |>
    select(names(leeg_verenigingen))
}

laad_verenigingen_data <- function(con) {
  heeft_raw_verenigingen <- dbExistsTable(con, DBI::Id(schema = "raw", table = "verenigingen"))
  heeft_raw_pricing <- dbExistsTable(con, DBI::Id(schema = "raw", table = "verenigingen_pricing"))
  
  if (heeft_raw_verenigingen) {
    verenigingen <- dbGetQuery(con, 'SELECT * FROM raw.verenigingen')
  } else if (file.exists("data/raw/Verenigingen.csv")) {
    verenigingen <- read.csv2("data/raw/Verenigingen.csv", stringsAsFactors = FALSE, check.names = FALSE)
  } else {
    verenigingen <- data.frame()
  }
  
  if (heeft_raw_pricing) {
    verenigingen_pricing <- dbGetQuery(con, 'SELECT * FROM raw.verenigingen_pricing')
  } else if (file.exists("data/raw/verenigingen_pricing.csv")) {
    verenigingen_pricing <- read.csv2("data/raw/verenigingen_pricing.csv", stringsAsFactors = FALSE, check.names = FALSE)
  } else {
    verenigingen_pricing <- data.frame()
  }
  
  maak_verenigingen_dataset(verenigingen, verenigingen_pricing)
}
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "verenigingen",
              ...
              br(),
              fluidRow(
                column(3, kpi_card("Verenigingen", textOutput("verenigingen_aantal"),
                                   subtitel = "unieke verenigingen")),
                column(3, kpi_card("Leden", textOutput("verenigingen_leden"),
                                   subtitel = "totaal aantal leden")),
                column(3, kpi_card("Members", textOutput("verenigingen_members"),
                                   subtitel = "totaal aantal members")),
                column(3, kpi_card("Omzet", textOutput("verenigingen_omzet"),
                                   subtitel = "via verenigingsdeals"))
              ),
              br(),
              fluidRow(
                box(width = 8, title = "Omzet per vereniging",
                    plotlyOutput("verenigingen_omzet_plot", height = "360px")),
                box(width = 4, title = "Members per sport",
                    plotlyOutput("verenigingen_sport_plot", height = "360px"))
              ),
              fluidRow(
                box(width = 6, title = "Leden versus members",
                    plotlyOutput("verenigingen_leden_members_plot", height = "340px")),
                box(width = 6, title = "Omzet door de tijd",
                    plotlyOutput("verenigingen_tijd_plot", height = "340px"))
              ),
              fluidRow(
                box(width = 6, title = "Performance per pricing code",
                    tableOutput("verenigingen_pricing_tabel")),
                box(width = 6, title = "Verenigingen overzicht",
...
server <- function(input, output, session) {
  ...
  verenigingen_samenvatting <- reactive({
    verenigingen_filtered() |>
      ...
      summarise(
        aantal_leden = max(aantal_leden, na.rm = TRUE),
        aantal_members = max(aantal_members, na.rm = TRUE),
        omzet = sum(omzet, na.rm = TRUE),
        discount = sum(discount, na.rm = TRUE),
        .groups = "drop"
      ) |>
      mutate(
        aantal_leden = ifelse(is.infinite(aantal_leden), 0, aantal_leden),
        aantal_members = ifelse(is.infinite(aantal_members), 0, aantal_members)
      )
  })
  
  output$verenigingen_aantal <- renderText({
    format_number(n_distinct(verenigingen_samenvatting()$vereniging_id))
  })
  
  output$verenigingen_leden <- renderText({
    format_number(sum(verenigingen_samenvatting()$aantal_leden, na.rm = TRUE))
  })
  
  output$verenigingen_members <- renderText({
    format_number(sum(verenigingen_samenvatting()$aantal_members, na.rm = TRUE))
  })
  
  output$verenigingen_omzet <- renderText({
    format_euro(sum(verenigingen_samenvatting()$omzet, na.rm = TRUE))
  })
  ...
  output$verenigingen_sport_plot <- renderPlotly({
    ...
    maak_donut_plot(plot_data, "sport", "aantal_members",
                    c(KLEUR_PRIMAIR, KLEUR_SECUNDAIR, KLEUR_TERTIAIR,
                      KLEUR_LICHT, "#9ca3af", "#d1d5db"))
  })
  
  output$verenigingen_leden_members_plot <- renderPlotly({
    plot_data <- verenigingen_samenvatting() |>
      arrange(desc(aantal_leden)) |>
      head(10)
  ...
  output$verenigingen_pricing_tabel <- renderTable({
    verenigingen_filtered() |>
      ...
      mutate(
        ...
        discount = format_euro(abs(discount))
      ) |>
      rename(
        "Pricing code" = pricing_code,
        "Vereniging deal" = vereniging_deal,
        "Omzet" = omzet,
        "Korting" = discount,
        "Regels" = regels
      )
